...
> 🔴 **换机提醒（2026-09-21）**：本机（`Unbox`）**尚未部署 BiliSum** —— `H:\Program\bilisum` 与运行数据 `C:\Users\Unbox\AppData\Local\bilisum\data` **都不存在**。
> 下面所有「双击启动」类说明在**重新部署前均不可用**；本文路径已按新机约定改写，作为重新部署的目标位置。
> 是否重新部署待用户决定，详见 `CHANGELOG.md`。
