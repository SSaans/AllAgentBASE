...
- 🔴 **结论：BiliSum 部署未迁移到新机**，旧机上的 `D:\Program\bilisum`、桌面入口、以及运行数据**在本机都不存在**。
- ✅ 已按新机约定把文档里的外部项目根路径改写为 `H:\Program\bilisum`（与 distilly / stickersync / AstrBot 保持一致），但**该目录目前不存在**，文档中所有「双击启动」类说明在重新部署前均不可用。
- ⚠️ 待用户拍板：① 是否在新机重新部署 BiliSum（含 LLM/ASR 配置与历史笔记恢复）；② 若不要了，本项目转为归档状态。
- 本条属历史事实勘误，不改写旧记录。
