...
> 👤 发起人：用户（Edi）
> 👤 维护者：规划 Agent
> 🎯 状态：任务 5 待 QQ 边界复验；任务 13 群漫画已真实出图并发群，待正式验收
...
| 转发卡片 | 🔄 30 字边界待复验 | 前轮只验证了 1500 字场景，不能证明用户要求的 30 字分界；本轮已把 QQ 档改为阈值 30、非流式、关闭自动分段，源码与实际配置测试通过 |
| 日志落盘 | ⚠️ 待重启验证 | `log_file_enable=true` 已保存但当前进程未动态加载文件 sink，需 Launcher 正常重启后核对 `core/data/logs/astrbot.log` |
| 隔离自测 | ✅ 10/10 通过 | `tests/check_installed_source.py`：白名单 / 人设回归 / 模型回退 / 定时关闭 / 29、30、31 字转发边界 / 实际 QQ 配置 / 转发节点到 OneBot 群合并转发动作；自动 LLM 链路的 QQ 展示仍待复验 |
...
| 5 | 旧阈值 1500 且流式回复开启 | 30 字以上内容仍被流式直发 | **已修复配置**：用户确认阈值 30；QQ 档设为非流式并关闭自动分段，待真实 QQ 30/31 字复验 |
