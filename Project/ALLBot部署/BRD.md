...
> 👤 发起人：用户（Edi）
> 👤 维护者：规划 Agent
> 🎯 状态：群分析、>30 字自动转发、群漫画均已真实群内成功；待规划 Agent 安排剩余边界与正式验收
...
| 长消息转发 | QQ 档 `forward_threshold = 30`、`streaming_response = false`、`segmented_reply.enable = false`；≤30 字单条直发，>30 字转合并转发卡片 |
...
| 转发卡片 | ✅ >30 字自动链路已实测；≤30 字边界待复验 | QQ 档已改为阈值 30、非流式、关闭自动分段；配置更新后，群消息历史确认 11:23:15、11:23:46、11:27:23 三次普通 LLM 长回复均为 `forward`。恰好 ≤30 字直发 1–2 条仍待正式群内边界复验 |
...
**机制（已确认存在）**：AstrBot 内置逻辑——非流式 LLM 回复的 Plain 文本总字符数**超过 `forward_threshold`（当前 30）**时，转为 QQ「合并转发聊天记录」卡片。流式结果会提前发送并跳过该判断，因此 QQ 档须保持 `provider_settings.streaming_response=false`。

**方案**：
- 用户确认标准：≤30 字直接发送 1–2 条普通消息，>30 字发送合并转发卡片。当前关闭 `segmented_reply`，使短回复稳定为 1 条普通消息；
...
- [ ] 短回复（≤30 字）以 1 条普通消息直接发送；长回复（>30 字）以合并转发聊天记录卡片发送，群内可正常点开查看
...
| 5 | 旧阈值 1500 且流式回复开启 | 30 字以上内容仍被流式直发 | **已修复并验证 >30 字路径**：QQ 档阈值 30、非流式、关闭自动分段；仅恰好 ≤30 字直发 1–2 条的真实 QQ 边界待复验 |
