...
- **确认项**：`group_list_mode = "none"` 且群列表为空——确认语义为"任意群可触发"即可，无需绑定群；`min_messages_threshold = 200` 消息不足时不产出报告（向群内说明原因）；
...
| 2 | 群分析插件 `llm_provider_id=""` | 插件 LLM 可能无法调用 | 实测，如失败则配置为默认 provider |
| 3 | 消息量不足（<200 条）不出报告 | 小群无法总结 | 向群内明确提示原因，必要时调低阈值 |
| 4 | 合并转发在 SnowLuma/OneBot 的兼容性 | 长回复转发失败 | 实测，失败则查协议端日志 |
