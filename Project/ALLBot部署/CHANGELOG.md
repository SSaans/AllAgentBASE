...
- 本子项目 CHANGELOG 两条历史记录均声明「归档 1 条至 `CHANGELOG.archive.md`，主文件保持 15 条上限」，但事实是：本文件由根拆分迁入后**仅 3 条**、远未触及 15 条上限；`Project/ALLBot部署/CHANGELOG.archive.md` **从未创建、当前不存在**；被声称归档的「2026-09-08 识屏误判压缩 Bug 回退」条目属 **shinsekai 子项目**，从不属于本子项目。已按「保留原文 + 追加勘误」在两条记录末尾加标注，未改写原文、未新建任何 MD 文件
...
> ⚠️ **勘误（2026-09-15 规划 Agent 交接轮补记；原文保留不改写）**：本条「最旧 1 条（2026-09-08 识屏误判压缩 Bug 回退）移入 `CHANGELOG.archive.md`，主文件保持 15 条上限」，以及「修改的文件」中列出的 `CHANGELOG.archive.md`，均**与事实不符**：① 被声称归档的那条属 **shinsekai 子项目**，从不属于 ALLBot部署；② 本文件由根 CHANGELOG 拆分迁入后**仅 3 条**，远未触及 15 条上限，无需归档；③ `Project/ALLBot部署/CHANGELOG.archive.md` **从未创建**（`git ls-files` 无此文件）。本子项目首次归档动作尚未发生。
