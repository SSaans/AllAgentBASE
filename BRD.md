...
> 📅 更新时间：2026-09-22  
> 👤 维护者：规划 Agent  
> 🎯 状态：已立项 7 个子项目 —— Shinsekai（维护）、ALLBot部署、bilisum部署（2026-09-17 部署完成，待测试 Agent 验收）、distilly（2026-09-20 立项，**阶段 1 环境部署已完成**，待提供材料进入阶段 2）、**表情包同步 StickerSync**（2026-09-20 立项，R2 聚焦 TIM + 微信，规划完成待用户放行）、**万有引力**（2026-09-20 立项，跨平台个人聊天记录归档，规划完成待用户拍板；⚠️ **本仓库为公开仓库，该子项目按保密要求脱敏登记**，见 `Project/万有引力/BRD.md` §七）、**中转站搭建**（2026-09-22 立项，本机 new-api 面板聚合上游中转站，**自用定位**，规划完成待用户提供上游坐标）
...
| ~~万有引力（跨平台聊天记录归档）~~ | 已立项为 `Project/万有引力`（2026-09-20）：把散落各平台的**本人**聊天记录收拢归一，导出为可长期保存的格式。**平台优先级：微信 P0 → QQ/TIM P1 → B站私信 P2 → 抖音 P3（不承诺）**。⚠️ 本仓库公开，坐标按保密要求不登记 | 已立项 |
| ~~中转站搭建（AI API 聚合面板）~~ | 已立项为 `Project/中转站搭建`（2026-09-22）：在本机 Windows 部署 **new-api** 面板，把手上的各家中转站 key 接为上游渠道，统一出口自用。**底座是 new-api 不是 sub2api**（理由见子项目 BRD §二）。🔴 边界：只监听本机、不对外、不收费；凭据一律不入库 | 已立项 |
| ~~CHANGELOG 归档工具~~ | 优先级待定，未立项 | 待定 |
| 文档一致性检查脚本 | 校验 BRD/CHANGELOG 格式与勾选项状态，可后续接入 CI | 待定 |

> ⚠️ 已立项子项目：`Project/shinsekai项目byendcycle`（Shinsekai 桌宠，2026-09-07，维护模式）、`Project/ALLBot部署`（AstrBot QQ 机器人，2026-09-15 立项，待用户确认后开发）、`Project/bilisum部署`（BiliSum 视频摘要工具，2026-09-17 立项，部署已完成待验收）、`Project/distilly`（蒸馏自己，2026-09-20 立项，规划完成待用户放行）、`Project/表情包同步`（表情包同步 StickerSync，2026-09-20 立项，规划完成待用户放行）、`Project/万有引力`（跨平台个人聊天记录归档，2026-09-20 立项，规划完成待用户拍板）、`Project/中转站搭建`（AI API 聚合面板，2026-09-22 立项，**自用定位**，规划完成待用户提供上游坐标）。上表为历史候选，不代表当前授权；新子项目由用户拍板后，规划 Agent 产出子项目 BRD，用户确认后开发 Agent 才能开始编码。
>
