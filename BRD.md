...
| 层 | 文件 | 回答什么问题 | 谁维护 |
|---|---|---|---|
| **L0 规范层** | `AGENTS.md`、`PLANNING.md` / `DEVELOPMENT.md` / `TESTING.md`、本文件「子项目需求规范」节 | 所有项目**怎么干**（流程 / 标准 / 模板 / 红线） | 规划 Agent |
| **L1 平台需求层** | **本文件（根 `BRD.md`）** | **平台自己**要什么（把平台当成一个产品） | 规划 Agent |
| **L2 子项目需求层** | `Project/<子项目>/BRD.md` | **某个子项目**要什么 | 规划 Agent 起草 → 用户确认 → 开发实现 |

**层级关系**：L0 约束 L1 与 L2（三个 Agent 都必须遵守）；L1 对 L2 **只做索引**；L2 套用 L0 的流程与 L1 的通用模板。
